...
The new local learning modules fit a small statistical context policy and persist frozen evaluation rounds, repeated-experiment risk accounting, activation and rollback. Run `python examples/local_learning.py` for the complete offline example. Its two-cell tasks and resource values are explicitly simulated; 24 boundary tests cover the learning controls, and the [full local engineering suite](evidence/local-learning-v1/engineering.json) passes 328 tests without skips. The [requirement register](evidence/enterprise-context-v1/requirements.json) separates implemented components from unfinished work. This is a development programme, not a 1.0 or AGI capability claim.
...
The [local coding-agent pilot](docs/terminal-local-pilot.md) now runs isolated Harbor environments and native Terminal-Bench 2.1 assertions. Qwen2.5 1.5B passed **0/2 selected development tasks** after a bounded action-interface correction. Native reference solutions passed and empty submissions failed. The failed first attempt, all six model calls, token counts and grader reports are retained. A local improvement policy cannot compensate for an agent that does not yet create valid submissions; this failure sets the next model/scaffold comparison. No benchmark material was used for training. The [evaluation programme](docs/evaluation-programme.md) separates these controls from future runtime treatments and local adaptation trials.

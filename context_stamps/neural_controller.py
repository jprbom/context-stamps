...
class RecurrentEvidenceRanker(nn.Module):
    ...
    def score_candidates(self, queries, documents, features, mask, *, steps=None):
        # More untrained recurrence caused severe public-data regressions. Keep
        # ablations in forward(); serving uses the checkpoint's trained depth.
        if steps is not None and (type(steps) is not int or steps != self.config["steps"]):
            raise ValueError("serving requires the checkpoint's trained recurrent depth")
        ...
        result = self(queries, documents, features, mask, steps=steps)
        if not torch.isfinite(result).all().item():
            raise ValueError("candidate arithmetic overflow")

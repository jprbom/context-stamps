...
Every supported edge deployment should expose a local adaptation cycle, even when its device cannot train language-model weights. On a CPU-only device, verified memory and the small statistical context policy can change locally. On a device with sufficient training memory, a host can also fit a small head or adapter while retaining an immutable base checkpoint. Candidate types and permitted training data are fixed by the host. A model's own fluent answer is not a verified label.

The intended cycle is: collect authorized outcomes, independently check them, reserve fresh evaluation tasks, fit a candidate, measure paired task success and total resource use, then retain the current version or promote the candidate with rollback available. A device must be allowed to conclude that no update improves its present policy. Weight updates need older-task retention and target-device inference checks as well as new-task gains. The current library implements parts of this cycle; it does not yet supply a qualified unattended weight-learning service for every SLM.
...
The [verified public-code learning experiment](verified-code-learning.md) adds a separate adapter trained on 368 qualified MBPP examples, without using HumanEval answers for fitting. Its training records and checkpoint are retained. Public benchmark results must be interpreted separately from local domain retention, memory-policy benefit and edge-device qualification; none is a substitute for the others.

...
The research worktree is isolated from the public main branch. The current full suite passes 290 tests without skips, including 23 incremental-computation tests. The [incremental layer](incremental-computation.md) passes 720 fictional workflow requests with exact outputs; local branch changes reuse unrelated work, while policy-wide changes force recomputation and add overhead. The [persistent context layer](persistent-context.md) retains its 267-test revision and 540 synthetic storage checks. Simple prefetch remains slower than working-set reuse alone and stays opt-in. The earlier temporal/compiler, audit and managed-execution reports retain their source-bound evidence and failures. These are engineering results; real decision calibration, learned sufficiency and public benchmark gains remain unproven.

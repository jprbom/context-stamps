...
| Learning/audit | Transactional reference-only journal, reserved outcome capacity, keyed receipts, historical source archives and exact reuse | Persistent context state, retention/export policy, prospective labels and policy candidate evaluation |
...
The research worktree is isolated from the public main branch. The current full suite passes 233 tests without skips. The [temporal state/compiler/decision layer](enterprise-state.md) has 80 synthetic compilation measurements; the [durable audit layer](durable-audit.md) adds 19 tests and 1,152 measured local appends. The [managed executor](managed-execution.md) adds 30 execution tests, two archive tests, a complete context-to-worker-to-typed-decision example and explicit measurements of process/journal overhead. Earlier runs and the mixed-clock measurement failure remain intact. These are engineering results; real decision calibration, learned sufficiency and public benchmark gains remain unproven.

The initial training-only RTX probe selected batch 128 for both existing controller variants; longer windows found substantial variation and no consistent BF16 benefit. Use the GPU for training and numerical/performance qualification; keep state, preparation and routine verification on CPU. Next: persistent context state and virtual memory/incremental computation, stronger worker containment/residency, and prospective datasets for a new context model. Preserve the failed student/router controls and all existing qualified retrieval routes.

...
| Learning/audit | Transactional reference-only journal, reserved outcome capacity, keyed receipts, historical source archives and exact reuse | Retention/export policy, prospective labels and policy candidate evaluation |
...
The research worktree is isolated from the public main branch. The current full suite passes 267 tests without skips, including 34 persistent-state/working-set tests. The [persistent context layer](persistent-context.md) also passes 540 exact checks across 27 synthetic storage streams. Working-set reuse reduces payload materializations in this fixture; simple prefetch is slower than working-set reuse alone at every tested repeat ratio and stays opt-in. The [temporal state/compiler/decision layer](enterprise-state.md), [durable audit layer](durable-audit.md) and [managed executor](managed-execution.md) retain their earlier source-bound evidence. Earlier runs and the mixed-clock measurement failure remain intact. These are engineering results; real decision calibration, learned sufficiency and public benchmark gains remain unproven.

The initial training-only RTX probe selected batch 128 for both existing controller variants; longer windows found substantial variation and no consistent BF16 benefit. Use the GPU for training and numerical/performance qualification; keep state, preparation and routine verification on CPU. Next: the expanded local benchmark programme, incremental computation, retention, stronger worker containment/residency, and prospective datasets for a new context model. Preserve the failed student/router controls and all existing qualified retrieval routes.

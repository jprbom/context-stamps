...
Timestamps are supplied by the trusted host. They do not prove that a prediction existed before its consequence. The future transactional ledger must enforce event order independently and record its own commit sequence/time. A predicted payload remains in separately authorized storage; its reference cannot appear in the observations field. A `verified=True` label is an assertion from the host verifier, not a new independent proof of truth.

Use opaque IDs, never credential strings or raw private text. Strict structure cannot detect a secret disguised as a valid identifier. Current authorization, retention, revision checks and export permissions remain host responsibilities until the corresponding store/runtime interfaces are implemented. Do not expose these constructors as an unauthenticated service.

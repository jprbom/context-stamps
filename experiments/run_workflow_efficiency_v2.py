...
from context_stamps import ContextGraph, ContextNode, StampSchema, activate_constrained
from experiments.run_local_tasks import MODEL, SYSTEM, api
from experiments.run_spherical import encode
from experiments.run_spherical_public import read, save, sha

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "evidence/workflow-efficiency-v2"
...
def build(case, method):
    ...
    return graph, revisions, metadata, candidates, elapsed(start)
...
def main():
    ...
    warmup = api("generate", {"model": MODEL, "prompt": "Return {}.", "stream": False,
                              "format": "json", "keep_alive": "10m", "options": {"num_predict": 8}})
        ...
        for case in cases:
            ...
            for stage in range(4):
                ...
                for method in methods:
                    graph, revisions, metadata, candidates, _ = states[method]
                    start = time.perf_counter_ns()
                    ...
                    if method == "spherical_guarded":
                        ...
                        transport_bytes = len(payload.encode())
                    ...
                    if packet.status == "complete":
                        task = ('Return JSON {"code": "..."} containing only two Python integer assignments: SETTING and LIMIT.'
                                if stage == 1 else 'Return JSON {"setting": number, "limit": number}.')
                        ...
                        response = api("generate", {"model": MODEL, "system": SYSTEM, "prompt": prompt,
                            "stream": False, "format": ({"type": "object", "properties": {"code": {"type": "string"}},
                                "required": ["code"], "additionalProperties": False} if stage == 1 else
                                {"type": "object", "properties": {"setting": {"type": "integer"}, "limit": {"type": "integer"}},
                    ...
                    correct = (packet.status != "complete") if should_abstain else (packet.status == "complete" and grade(output, expected, stage == 1))
                    rows.append({"workflow": case["workflow"], "repeat": repeat, "stage": stage, "method": method,
                                 ...
                                 "prompt": prompt, "output": output, "end_to_end_ms": elapsed(start), "update_ms": update_ms,
                                 ...
                                 "context_bytes": packet.units, "transport_bytes": transport_bytes})
    ...
    save(OUT / "manifest.json", {"protocol": protocol, "model_metadata": metadata, "ollama_version": api("version"),
                                 ...
                                 "source_sha256": {p: sha(ROOT / p) for p in ("experiments/run_workflow_efficiency_v2.py",

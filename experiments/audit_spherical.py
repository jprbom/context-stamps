"""Activation, metadata control, transport accounting and live local SLM follow-up."""
...
OUT = ROOT / "evidence/spherical-audit-v1"
...
def main():
    ...
    for repetition in range(2):
        for task in tasks:
            modes = ["full", "stamp_only", "activated_graph"]
            rng.shuffle(modes)
            for mode in modes:
    ...
    save(OUT / "slm-summary.json", [{"mode": mode, "calls": 24, "unique_cases": 12, **{
        ...
        for mode in ("full", "stamp_only", "activated_graph")])
    sources = ["context_stamps/activation.py", "experiments/audit_spherical.py", "experiments/run_spherical_v2.py"]

...
class ContextSession:
    ...
    def _remove(self, token):
        key, packet, _ = self._entries.pop(token)
        del self._keys[key]
        self._bytes -= packet.units

    def _expire(self):
        now = time.monotonic()
        for token, (_, _, expires) in list(self._entries.items()):
            if expires <= now:
                self._remove(token)
    ...
    def issue(self, roots, *, role, revisions, budget_bytes=8192):
        ...
        key = (tuple(sorted(set(roots))), role, tuple(sorted(revisions.items())), budget_bytes)
        with self._lock:
            self._expire()
            ...
            while token in self._entries:
                token = secrets.token_bytes(32)
            self._entries[token] = key, packet, time.monotonic() + self._ttl
